...
void blocks_mesh_update(struct blocks_mesh *blocks_mesh, struct blocks_vertices vertices)
{
  ...
  // Vertex attributes
  {
    ...
    glVertexAttribIPointer(0, 3, GL_UNSIGNED_INT, sizeof(struct blocks_vertex), (void *)offsetof(struct blocks_vertex, center));
    glVertexAttribIPointer(1, 1, GL_UNSIGNED_INT, sizeof(struct blocks_vertex), (void *)offsetof(struct blocks_vertex, normal_index_and_texture_index));
    glVertexAttribIPointer(2, 1, GL_UNSIGNED_INT, sizeof(struct blocks_vertex), (void *)offsetof(struct blocks_vertex, light_level_and_occlusion_counts));
    glVertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, sizeof(struct blocks_vertex), (void *)offsetof(struct blocks_vertex, damage));
