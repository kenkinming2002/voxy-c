...
voxy_entity_id_t voxy_entity_registry_register_entity(struct voxy_entity_registry *registry, struct voxy_entity_info entity_info)
{
  const voxy_entity_id_t id = registry->infos.item_count;
  DYNAMIC_ARRAY_APPEND(registry->infos, entity_info);
  return id;
}

struct voxy_entity_info voxy_entity_registry_query_entity(struct voxy_entity_registry *registry, voxy_entity_id_t id)
{
