  ...
  // 1: Chunk Rendering
  {
    world_for_each_chunk(chunk)
      if(chunk->render_info)
      {
        ...
        chunk->render_info->culled = (min.x >= 1.0f || max.x <= -1.0f)
    ...
    world_for_each_chunk(chunk)
      if(chunk->render_info)
        if(!chunk->render_info->culled && chunk->render_info->opaque_mesh.count != 0)
        {
          glBindVertexArray(chunk->render_info->opaque_mesh.vao);
          glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_BYTE, 0, chunk->render_info->opaque_mesh.count);
        }

    world_for_each_chunk(chunk)
      if(chunk->render_info)
        if(!chunk->render_info->culled && chunk->render_info->transparent_mesh.count != 0)
        {
          glBindVertexArray(chunk->render_info->transparent_mesh.vao);
          glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_BYTE, 0, chunk->render_info->transparent_mesh.count);
        }
  }
