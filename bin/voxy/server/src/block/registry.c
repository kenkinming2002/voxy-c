...
voxy_block_id_t voxy_block_registry_register_block(struct voxy_block_registry *registry, struct voxy_block_info block_info)
{
  const voxy_block_id_t id = registry->infos.item_count;
  DYNAMIC_ARRAY_APPEND(registry->infos, block_info);
  return id;
}

struct voxy_block_info voxy_block_registry_query_block(struct voxy_block_registry *registry, voxy_block_id_t id)
{
