...
int gl_shader_load(struct gl_shader *shader, GLenum target, const char *filepath)
{
  ...
  if(!success)
  {
    ...
    LOG_ERROR("Failed to compile shader: %s", info_log);
    free(info_log);
...
int gl_program_link(struct gl_program *program, size_t count, struct gl_shader shaders[count])
{
  ...
  if(!success)
  {
    ...
    LOG_ERROR("Failed to link program: %s", info_log);
    free(info_log);
...
int gl_texture_2d_load(struct gl_texture_2d *texture_2d, const char *filepath)
{
  ...
  if(!bytes)
  {
    LOG_ERROR("Failed to load image for texture from %s: %s", filepath, stbi_failure_reason());
...
int gl_array_texture_2d_load(struct gl_array_texture_2d *array_texture_2d, size_t count, const char *filepaths[count])
{
  ...
  for(size_t i=0; i<count; ++i)
  {
    ...
    if(!bytes)
    {
      LOG_ERROR("Failed to load image for array texture from %s: %s", filepaths[i], stbi_failure_reason());
    ...
    if(width != x || height != y)
    {
      LOG_ERROR("Array texture must be comprised of images of same dimension: Expected %dx%d: Got %dx%d from %s", width, height, x, y, filepaths[i]);
